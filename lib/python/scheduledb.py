    ...
    if cn==0:
      ...
      for field in ScheduleDB.SCHEDULE_INSERT_COLUMNS:
        try:
          ...
          self.warn("event: {}".format(event))
          self.warn("has no field {}".format(field))
          tmp=''
        except Exception as e:
          self.error("error in formatfield")
          self.error("field: {}".format(field))
          self.error("event: {}".format(event))
          self.error("exception type: {}".format(type(e)))
          self.error("args: {}".format(e.args))
          raise
        try:
          sql += tmp + ','
