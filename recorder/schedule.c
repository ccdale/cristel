 ...
 * schedule.c
 *
 * Last Modified: Friday  7 October 2016, 15:05:09
 *
...
void getepg(void)/* {{{1 */
{
    char lookfor[]="Finished updates\0";
    FILE *fp;
    int status;
    char buffer[1024];

    INFO("starting epg update");
    fp=popen("/usr/local/bin/getepg -c","r");
    if(fp!=NULL){
        ...
        status=pclose(fp);
        if(status==-1){
            WARN("%d Error reported by pclose for epg update: %s",errno,strerror(errno));
        }
    }else{
        WARN("Failed to execute /usr/local/bin/getepg");
    }
